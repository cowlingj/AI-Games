...
_hidden_count = 5
_layers = [ nn.Linear(utils.num_features, _hidden_size), nn.Tanh() ] + \
    [ x for xs in [ [nn.Linear(_hidden_size, _hidden_size), nn.Tanh(), nn.Dropout() ] for _ in range(_hidden_count) ] for x in xs ] + \
