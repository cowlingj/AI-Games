...
def encode_plan(state: np.ndarray, player: int, col: int):
    move = np.zeros(state.shape[1], dtype=np.int8)
    move[col] = 1
    return np.concatenate((np.reshape(state, -1), [player], move), dtype=np.int8)
...
with open("training.txt", "at") as out:
    while True:
        ...
        np.savetxt(out, encode_plan(state, 1 if moves % 2 == 0 else -1, col - 1), newline=",", fmt="%1d")
        print("", file=out)
        out.flush()
